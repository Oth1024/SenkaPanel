...
impl ConfigTool {
    ...
    pub fn initialize(&mut self) {
        read_default_config(&mut self.config_infos);
    }

    pub fn regist_config<T>(&self) {}

    pub fn read_config<T>(&self) -> T {}

    pub fn set_config<T>(&self, new_config_object: &T) {}
}
...
async fn read_default_config(config_infos: &mut HashMap<String, ConfigInfo>) {
            ...
            if let Ok(content_str) = content {
                ...
                if let Ok(toml_content) = get_toml {
                    for config_name_and_config in toml_content.iter(){
                        config_infos.insert(String::from(config_name_and_config.0), config_name_and_config.1.clone());
                    }
                }
                else {
