...
static CONFIGS: Lazy<RwLock<HashMap<&'static str, Arc<dyn Any + Send + Sync>>>> = Lazy::new(|| RwLock::new(HashMap::<&'static str, Arc<dyn Any + Send + Sync>>::new()));

/// 获取配置，当不存在配置的时候会尝试创建默认配置
pub async fn get_config<T: SenkaConfig + 'static + Serialize + DeserializeOwned>() -> Result<Arc<T>, SenkaError> {
    match get_config_from_cache::<T>().await {
        Ok(config) => Ok(config),
        Err(_) => match read_config_string_from_disk::<T>().await {
            Ok(config_string) => match toml::from_str::<T>(config_string.as_str()) {
                Ok(config_from_disk) => {
                    ...
                    CONFIGS.write().await.insert(config_name, config_arc);
                    get_config_from_cache::<T>().await
                }
            ...
            Err(_) => {
                ...
                match toml::to_string::<T>(default_config.as_ref()) {
                    Ok(config_string) => {
                        match update_config_to_disk::<T>(config_string).await {
                            Ok(_) => {
                                CONFIGS
                                    .write()
                                    .await
...
/// 从内存中读取之前存储的配置
pub async fn get_config_from_cache<T: Any + 'static>() -> Result<Arc<T>, SenkaError> {
    let config_name = type_name::<T>();
    match CONFIGS.read().await.get(config_name) {
...
/// 更新存储在内存中的配置
pub async fn update_config_cache<T: Any + Send + Sync + 'static>(new_config: Arc<T>) {
    let config_name = type_name::<T>();
    CONFIGS.write().await.insert(config_name, new_config);
}
